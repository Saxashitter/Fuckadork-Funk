...
InputEventTouch = crequire("input.touch.InputEventTouch") --- @type chip.input.mouse.InputEventMouse
...
Chip.mobile = love._os == "Android" or love._os == "iOS"

local plugins = Engine.plugins
local function physics()
	Engine.prePhysics:emit((1/Engine.targetFPS)/Engine.physicsPerFrame)

	if Engine.prePhysics._cancelled then
		Engine._currentPhysicsTime = 0
		return true
	end

	Engine.currentScene:physics((1/Engine.targetFPS)/Engine.physicsPerFrame)
	Engine._currentPhysicsTime = Engine._currentPhysicsTime - (1/Engine.targetFPS)/Engine.physicsPerFrame

	Engine.postPhysics:emit((1/Engine.targetFPS)/Engine.physicsPerFrame)
end
local function update(dt)
    Engine.preUpdate:emit(dt)
    ...
    if Engine.currentScene then
		Engine._currentPhysicsTime = Engine._currentPhysicsTime + (dt * Engine.timeScale)

		while Engine._currentPhysicsTime >= (1/Engine.targetFPS)/Engine.physicsPerFrame do
			if physics() then break end
		end
...
local function touchpressed(id, x, y)
	if not Chip.mobile then return end

    local event = InputEventTouch:new(id, x, y, true)
    processInputEvent(event)
    event:free()
end
local function touchreleased(id, x, y)
    ...
    local event = InputEventTouch:new(id, x, y, false)
    processInputEvent(event)
    event:free()
end
local function resize(width, height)
	if Engine.noBorders then
		Engine.gameWidth = width
		Engine.gameHeight = height
		shove.setResolution(width, height)

		if Engine.currentScene then
			Engine.currentScene:resize(width, height)
		end
	end
...
function Chip.init(settings)
    love.load = function(args)
		...
		local gameWidth = settings.gameWidth or Engine.gameWidth
		local gameHeight = settings.gameHeight or Engine.gameHeight
	
		local referenceWidth = gameWidth
		local referenceHeight = gameHeight

		if settings.noBorders then
		...
		Engine.referenceWidth = referenceWidth
		Engine.referenceHeight = referenceHeight

		Engine.noBorders = settings.noBorders

        Engine.targetFPS = settings.targetFPS
        Engine.physicsPerFrame = settings.physicsPerFrame
